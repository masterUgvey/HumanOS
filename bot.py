...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "my_quests":
        ...
        if not quests:
            ...
            keyboard = []
            for quest in quests:
                # Кнопка перехода в квест (current_value=quest[5], target_value=quest[4])
                qid = quest[0]
                deadline_info = ""
                if len(quest) > 7 and quest[7]:
                    ...
                    try:
                        ...
                        deadline_info = f" 📅 {d.strftime('%d.%m.%Y')}"
                    except Exception:
                        pass
        ...
        # Меню выбора типа квеста
        context.user_data["creating_quest"] = True
        # Сбрасываем промежуточные состояния, если пользователь начал заново
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(text, reply_markup=reply_markup)
    elif query.data.startswith("quest_progress_"):
        # Запросить у пользователя величину частичного прогресса
        user_id = query.from_user.id
        try:
    ...
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        # Сохраняем выбранный тип и переводим в ожидание названия квеста
    type_map = {
